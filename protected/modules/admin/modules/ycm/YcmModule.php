...
class YcmModule extends CWebModule
{
	...
	public function loadModel($name,$pk=null)
	{
		...
		$model->attachBehavior('admin',array('class'=>'application.modules.admin.modules.ycm.behaviors.FileBehavior'));
		return $model;
	}
	...
	public function init()
	{
		...
		Yii::app()->setComponents(array(
			'errorHandler'=>array(
				'errorAction'=>'admin/'.$this->name.'/default/error',
			),
		), true);
	}
		...
		switch ($widget) {
			...
			case 'file':
				...
				if (!$model->isNewRecord && !empty($model->$attribute)) {
					...
					$this->controller->widget('bootstrap.widgets.TbButton',array(
						...
						'type'=>'',
						'url'=>yii::app()->baseUrl.$model->$attribute,
					));
					echo '</p>';
					$html=ob_get_clean();
					$options['hint']=$html;
			...
			case 'image':
				...
				if (!$model->isNewRecord && !empty($model->$attribute)) {
					...
					$image=CHtml::image(yii::app()->baseUrl.$model->$attribute,Yii::t('YcmModule.ycm','Image'),array(
						'class'=>'modal-image')
					);
					ob_start();
