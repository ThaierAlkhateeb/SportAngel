...
class ModelController extends AdminController
{
	...
	public function actionList($name)
	{
		...
		$this->breadcrumbs=array(
                        Yii::t('AdminModule.admin','Content Management')=>yii::app()->createUrl('admin/ycm/'),
		...
		$urlPrefix='Yii::app()->createUrl("admin/'.$this->module->name.'/model/';
		$data2=array(
			'id'=>'objects-grid',
				...
				if ($widget=='file' || $widget=='image') {
					...
					if ($file instanceof CUploadedFile) {
						...
						array_push($paths,$path);
                                                $url = '/uploads/'.$name.'/'.$attribute.'/'.$fileName;
						$model->$attribute=$url;
					}
				}
			}
			...
			foreach ($model->tableSchema->columns as $column) {
				...
				if ($widget=='file' || $widget=='image') { // file or image
					...
					if (isset($model->$attribute)) {
						$path=Yii::getPathOfAlias('webroot').$model->$attribute;//$attributePath.DIRECTORY_SEPARATOR.$model->$attribute;
						if (file_exists($path)) {
	...
	public function actionExcel($name)
	{
        ...
        public function beforeAction($action) {
            ...
            if(Yii::app()->user->checkAccess('Admin:Admin_ycm:Model:'.$action->id))
            {
                return true;
            }
            else
